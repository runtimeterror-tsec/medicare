...
function viewPatients() {
    ...
    return firebase.database().ref("Users/" + firebase.auth().currentUser.uid + '/Patients').once('value').then(function (snapshot) {
        ...
        $.each(patientsArray, function () {
            ...
            rows += "<tr><td>" + patientEmail + "</td><td><button class='btn btn-primary ml-2' onclick='getReport(" + (i+1) + ")'>View Report</button></td><td><button id='loginModalOpen' onclick='updateReport(" + (i + 1) + ")' class='btn btn-primary ml-3'>Update Report</button></td><td><input id='fileid' name='fileid' type='file' hidden /></td></tr>";
            i = i + 1;
        });
...
function getReport(i) {
    ...
    console.log("Here" + clickedPatient);
