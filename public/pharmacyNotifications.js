    ...
    return firebase.database().ref("Users/" + firebase.auth().currentUser.uid + '/Notifications').once('value').then(function(snapshot) {
        ...
        snapshot.forEach(function(userSnapshot) {
            ...
            $.each(insuranceNotificationsArray, function(){
                ...
                rows += "<tr><td>" + notification + "</td><td><button class='btn btn-primary' onclick='getNotificationsReport("+(i+1)+")'>View Report</button></td><td><button class='btn btn-success' onclick='sendNotification()'>Accept</button></td><td><button class='btn btn-danger' onclick='rejectNotification()'>Reject</button></td></tr>";
                i = i + 1;
            });
    ...
    return firebase.database().ref("Users/" + firebase.auth().currentUser.uid + '/Notifications').once('value').then(function(snapshot) {
        ...
        snapshot.forEach(function(userSnapshot) {
            ...
            firebase.database().ref('Users/' + userSnapshot.key + '/Notifications').once('value').then(function(userrsnapshot) {
                ...
                ref.update({
                    [userrsnapshot.numChildren()] : "Pharmacy " + firebase.auth().currentUser.email + " has accepted your medicines order."
                })
            })
        ...
        firebase.database().ref("Users/" + firebase.auth().currentUser.uid + '/Notifications').once('value').then(function(snapshot) {
            ...
            snapshot.forEach(function(userSnapshot) {
                ...
                firebase.database().ref("Users/" + userSnapshot.key).once('value').then(function(snapshott) {
                    ...
                    firebase.database().ref('Users/' + firebase.auth().currentUser.uid + '/Order').once('value').then(function(count) {
                        var ref = firebase.database().ref(
                            "Users/".concat(firebase.auth().currentUser.uid, "/Orders")
                        );
                        ref.update({
    ...
    return firebase.database().ref("Users/" + firebase.auth().currentUser.uid + '/Notifications').once('value').then(function(snapshot) {
        ...
        snapshot.forEach(function(userSnapshot) {
            ...
            firebase.database().ref('Users/' + userSnapshot.key + '/Notifications').once('value').then(function(userrsnapshot) {
                ...
                ref.update({
                    [userrsnapshot.numChildren()] : "Pharmacy Firm " + firebase.auth().currentUser.email + " has rejected your medicines order."
                })
            })
